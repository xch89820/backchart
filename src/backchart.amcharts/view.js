...
*     Last Modified       :     [2014-03-09 19:43]
*     Description         :     Backchart view for amcharts
**********************************************************************************/
(function(root, name, factory) {
	"use strict";
	if (typeof define === 'function' && define.amd) {
		define(['jquery','../backchart.base/view','backbone',"underscore","AmCharts"], function($, base, Backbone, _, amcharts) {
			amcharts = amcharts || window.AmCharts;
			return factory($, Backbone, _, base, amcharts);
		});
		...
		module.exports = factory($, Backbone, _, base, amcharts);
	}else{
	...
	var backchartAmchartsView = baseview.extend(
		...
		constructor: function() {
            ...
            /**
             * The default categoryAxis when value is Date
             */
            this.defaultDateAxesOptions = {
                equalSpacing: true
            };
			 /**
			  * The default make chart Options
			  * @type {object}
			  * @memberof module:amcharts/view
			  */
			this.defaultOptions = {
		...
		/*__dealConflict: function(data, graphFields, mergeData){
            for (var d in data){
                if (mergeData[d]){
                    //has exist the attribute
                    var newName = _.uniqueId(d);
                    mergeData[newName] = data[d];
                    //change the graph field setting
                    for (var f in graphFields){
                        if (graphFields[f] === d){
                            graphFields[f] = newName;
                        }
                    }
                }else{
                    mergeData[d] = data[d];
        ...
        /**
         * A collection's data merge with other by key(category field)
		 * @private
         * @param key
         * @param data
         * @param dataProvider
			...
			me.eachCollection(function(uid, collection, renderOption){
					...
					if (collection.isVisibled(model)){
						...
						if (renderOptions.type === "serial" && (model.dataDateFormat || renderOptions.dataDateFormat)){
							if (!renderOptions.categoryAxis || typeof renderOptions.categoryAxis.parseDates === "undefined"){
								me._log().warn("One model's category field is Date Object, but not set categoryAxis.parseDates");
							}else{
								var dataDateFormat = model.dataDateFormat || renderOptions.dataDateFormat;
                                if (typeof _d[keyFields[0]] === "undefined"){
                                    me._log().warn("One model's categoryField is undefined!!");
                                    return;
                                }
				...
				if (renderOption.axes){
					if (me.multiValueAxes === true){
						...
						me._log().info("We has ignored one valueAxes configuration because the multiValueAxes option is false.");
					}
				}
				graphs.push(_gph);
			});
